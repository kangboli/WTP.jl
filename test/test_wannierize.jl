...
function spread(gauge_matrix::Matrix)
    U[gamma_point][:, :] = gauge_matrix[:, :]
    M = gauge_transform(neighbor_basis_integral(scheme), U)
    return second_moment(M, scheme, 1) - norm(center(M, scheme, 1))^2
end

spread(U[brillouin_zone[0,0,0]])

optimize(spread, U[brillouin_zone[0,0,0]], GradientDescent(manifold=Stiefel()), autodiff = :forward)

center(M, scheme, 1)

second_moment(M, scheme, 1)

u_1 = U[brillouin_zone[0, 0, 0]]

using LinearAlgebra

gradient(u->tr(u_1, u), u_1)
